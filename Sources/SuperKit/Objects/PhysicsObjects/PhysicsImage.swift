...
public class PhysicsImage: Image, Physics {
    
    private(set) public var __physics__: SKPhysicsBody!
    override var type: NodeTypes { .PhysicsImage }
    
    @discardableResult
    public override init(_ named: String, _ execute: (() -> ())? = nil) {
        super.init(named)
        __physics__ = SKPhysicsBody.init(texture: __sprite__.texture!, size: __sprite__.size)
        update()
        s(execute)
    }
