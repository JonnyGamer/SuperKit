...
public class PhysicsBox: Box, Physics {
    
    private(set) public var __physics__: SKPhysicsBody!
    override var type: NodeTypes { .PhysicsBox }
    
    @discardableResult
    public override init(width: Double, height: Double,_ execute: (() -> ())? = nil) {
        __physics__ = SKPhysicsBody.init(rectangleOf: .ratio(width, height))
        super.init(width: width, height: height, execute)
        update()
    }
    
    @discardableResult
    public override init(image: String,_ execute: (() -> ())? = nil) {
    ...
    required public init(from decoder: Decoder) throws {
        ...
        __physics__ = SKPhysicsBody.init(rectangleOf: .ratio(self.width, self.height))
