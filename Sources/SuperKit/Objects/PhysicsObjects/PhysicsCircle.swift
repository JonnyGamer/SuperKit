...
public class PhysicsCircle: Circle, Physics {
    
    private(set) public var __physics__: SKPhysicsBody!
    override var type: NodeTypes { .PhysicsCircle }
    
    
    @discardableResult
    public override init(radius: Double,_ execute: (() -> ())? = nil) {
        __physics__ = SKPhysicsBody.init(circleOfRadius: radius)
        super.init(radius: radius, execute)
        update()
    }
